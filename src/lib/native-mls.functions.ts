...
const b64 = z.string().min(1).max(16_384).regex(/^[A-Za-z0-9+/=_-]+$/);
...
export const ackMlsEnvelope = createServerFn({ method: 'POST' })
  .middleware([requireSupabaseAuth])
  .inputValidator((data: unknown) =>
    z.object({ envelopeId: z.string().uuid() }).parse(data),
  )
  .handler(async ({ data, context }) => {
    const { data: ok, error } = await rpcClient(context.supabase).rpc(
      'ack_mls_envelope',
